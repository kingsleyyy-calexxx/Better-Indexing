    ...
    for root, dirs, files in os.walk(directory):
        for file_name in files:
            if file_name.endswith(".html"):
                ...
                # Extract document number from the file name
                doc_number = re.sub(r'\.html$', '', file_name)

                # Check if TF-IDF calculation was successful
                if tfidf_matrix is not None and feature_names is not None:
                    ...
                    if not indexed_pages_collection.find_one({"link": link}):
                        ...
                        indexed_pages_collection.insert_one({"link": link, "file_name": file_name})
                else:
                    print(
                        f"Skipping TF-IDF calculation for {html_file_path} as all documents are empty or contain only stop words.")
